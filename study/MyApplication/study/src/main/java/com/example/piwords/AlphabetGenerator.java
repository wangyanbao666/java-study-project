...
public class AlphabetGenerator {
    ...
    public static char[] generateFrequencyAlphabet(int base,
        ...
        double[] charPDF = new double[26];
        for (int i=0; i<26; i++){
            charPDF[i] =  ((double) charCount[i]/total);
//            System.out.println(charPDF[i]);
        }
        for (int i=1; i<26; i++){
            charPDF[i]+=charPDF[i-1];
//            System.out.println(charPDF[i]);
        }

        double[] charCDF = new double[26];
        for (int i=0; i<26; i++){
            charCDF[i] =  charPDF[i]*base;
            System.out.println(charCDF[i]);
        }
        ...
        int start = 0;
        for (int i=0; i<base;){
            if (charCDF[start]>(double) i){
                output[i] = (char) (start+97);
                i++;
            }else {
                start++;
            }
    ...
    public static void main(String[] args) {
        char[] test = generateFrequencyAlphabet(26, new String[]{"a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"});
        for (char c:test){
